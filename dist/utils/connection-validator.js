...
const connector_factory_1 = require("../factories/connector-factory");
const types_1 = require("../types");
class ConnectionValidator {
    /**
     * Validate tenant configuration
     */
    static validateTenantConfig(config) {
        try {
            ...
            if (!connector_factory_1.ConnectorFactory.isDatabaseTypeSupported(config.databaseType)) {
                throw new Error(`Unsupported database type: ${config.databaseType}`);
            }
            // Validate credentials
            connector_factory_1.ConnectorFactory.validateCredentials(config.databaseType, config.credentials);
            return true;
        }
        catch (error) {
     ...
     */
    static validateConnectionOptions(options) {
        try {
     ...
     */
    static validateMongoDBCredentials(credentials) {
        try {
     ...
     */
    static validatePostgreSQLCredentials(credentials) {
        try {
            // Basic validation is handled by validateTenantConfig
     ...
     */
    static validateMultipleTenantConfigs(configs) {
        try {
     ...
     */
    static isValidConnectionString(connectionString, databaseType) {
        try {
